...
```{r brms, fig.cap = "Robust conditional effects from the Bayesian model of the \"gold standard\", semisupervised and automatic methods at 89% credibility"}
...
Another limitation is the sample size of 100. But the limited sample size can have two different implications: 1) whether we have enough variety in articles (e.g. variations in vocabulary, stylistic clues, angles) to use any of the unsupervised or semi-supervised methods and 2) statistical power of the analysis and/or whether we have enough articles to use any of the unsupervised or semi-supervised methods. Statistically speaking, increasing the sample size does not always increase variety (if variety means variance, increasing the sample size tends to decrease the variance). In our opinion, a more reliable way to increase the variety of articles is not just to increase the sample size but also to increase the content categories or authors. With our current data, it is not possible to simulate the possible effect of increasing variety and this warrants further studies.
...
Another issue with the sample size is that unsupervised and semi-supervised methods studied in this benchmark might not work well with a sample size of 100. In the online appendix, an analysis is presented to simulate the possible effect of increasing the sample size on H1 and H2. Our simulation points to the direction that both H1 and H2 are more likely to be true when the sample size increases. Therefore, unsupervised methods such as ANTMN are more likely to detect **topics** rather than frames when the sample size increases. But we maintain that this finding needs to be confirmed with actual data in a new empirical study.
