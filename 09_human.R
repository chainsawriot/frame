...
max(.match_topics(topic_vector, frame_corpus))
...
## set.seed(12121)
## bind_rows(ireadRDS("expert_accuracy.RDS"), ireadRDS("human_accuracy.RDS")) %>% mutate(expert = c(rep(TRUE, 7), rep(FALSE, 5))) %>% arrange(maxp) %>% mutate(desc = reorder(desc, maxp)) %>% uncount(300) %>% mutate(value = rbinom(n(), 100, maxp) / 100) %>% ggplot(aes(x = value, y = desc, fill = expert)) + geom_density_ridges(alpha = 0.5, linetype = "blank") + xlim(0, 1) + geom_vline(aes(xintercept = 0.2, alpha = 0.5), linetype = "dashed") + geom_vline(aes(xintercept = 0.3, alpha = 0.5), linetype = "dashed") + xlab(expression(CCR[max])) + ylab("Treatment") + scale_fill_brewer(palette="Dark2") +  theme_minimal() + theme(legend.position = "none")
