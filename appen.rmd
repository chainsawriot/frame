...
From our 100 articles, we created further synthetic articles following the principle of bootstrapping. We synthesized new article based on the following algorithm:

1. Randomly select one article
2. Tokenize this article into *n* sentences
...
We repeat the above process for 500, 1000, and 2000 times to generate four different corpora. This approach is compatible with the bag-of-words representation used in all unsupervised and semi-supervised methods. Also in step 3, topical and frame clues have the same natural chance of being selected. To put this simulation in another perspective, it simulates whether frames, rather than topics, are more likely to be picked up by these unsupervised and semi-supervised methods when the sample size increases.
...
From this simulation, there is highly unlikely that increasing the sample size would increase the correct classification rate for all unsupervised methods. Instead, all methods, except STM, perform more like the de-facto null (0.3) with the increasing sample size. Therefore, these methods appear to be more keen to pick up **topics**, rather than frames, when sample size increases. It reflects by the performance curve rescinding towards the null value with the highest sample size. For the two semi-supervised methods, increasing the sample size appears to provide better resistance against the rescinding towards the null value.
...
```{r simbrms, fig.cap = "Robust conditional effects from the Bayesian model of the \"gold standard\", semisupervised and automatic methods at 89% credibility with different simulated sample sizes"}
